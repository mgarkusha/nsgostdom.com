	...
	class picture2 {
		...
		function display2(){
			...
			if($row[$this->name]=='' || vars('e')=='n'){
				echo "<br><font class=text>фотография отсутствует</font>";
			}else{
				echo "<a href='?e=".vars('e')."&rempic=".$this->name."' class=link>удалить</a>";
				if($this->display_pic){
					echo "<br>";
					echo "<a href='javascript: ".cr_href($this->path.$row[$this->name].'.jpg')."' class=link><img src='".conf::$bpath.$this->path.$row[$this->name].'small.jpg'."' hspace=0 vspace=0 border=0></a>";
				}else{
		...
		function write_file(){
			if(vars($this->name)=='replace'){
				if(vars('e')!='n'){
					...
					if(!empty($row[$this->name])) unlink(conf::$bpath.$this->path.$row[$this->name].'small.jpg');
					if(!empty($row[$this->name])) unlink(conf::$bpath.$this->path.$row[$this->name].'.jpg');
				}; 
				$t=explode('.',$_FILES[$this->name]['name']);
				$ext=$t[count($t)-1];
				//
