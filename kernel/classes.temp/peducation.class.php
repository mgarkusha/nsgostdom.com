<?php if (!defined('SCRIPTACCESS')) exit;
	class peducation {
		...
		public function work(){
			...
			global $auth;
			$db=mysql::query("SELECT * FROM #resume_education WHERE account='?' ORDER BY `id`",$auth->logged());
			$i=0;
		...
		public function store(){
			...
			$speciality=$HTTP_POST_VARS['speciality'];
			$level=$HTTP_POST_VARS['level'];
			$type=$HTTP_POST_VARS['type'];
			$year1=$HTTP_POST_VARS['year1'];
			$year2=$HTTP_POST_VARS['year2'];
			global $auth;
			$db=mysql::query("DELETE FROM #resume_education WHERE account='?'",$auth->logged());
			foreach($place as $i=>$j){
				mysql::query("INSERT INTO #resume_education (`account`,`place`,`speciality`,`level`,`type`,`year1`,`year2`) VALUES ('?','?','?','?','?','?','?');",array($auth->logged(),$place[$i],$speciality[$i],$level[$i],$type[$i],$year1[$i],$year2[$i]));
