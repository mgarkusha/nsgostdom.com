<?php if (!defined('SCRIPTACCESS')) exit;
	class table2_ {
		...
		function store(){
			...
			if(vars('e')=='n'){
				...
				if((int)$e_id>0){
					for($i=0;$i<count($this->fields);$i++){
						$sql="UPDATE ".$this->name." SET `".$this->fields[$i]->name."`='".addslashes(vars($this->fields[$i]->name))."' WHERE ".$this->primary."='".addslashes(vars('e'))."'";
						mysql_query($sql);
						//echo $sql."<br>";
					};
					//exit;
			...
			if(!empty($this->after_store)) eval(ereg_replace("#",vars('e'),$this->after_store));
		}
		//
		function delete($id){
			if((int)$id<=0) return;
			...
			if(!empty($this->before_delete)) eval(ereg_replace("#",$id,$this->before_delete));
				...
				if(!$f){
					...
					if(ereg('varchar',strtolower($row['Type']))){
						if(	strtolower($this->fields[$i]->MySQLType)!=strtolower($row['Type'])
							&& strtolower($this->fields[$i]->MySQLType)!=ereg_replace('varchar','char',strtolower($row['Type']))
						) $tf=true;
					}elseif(ereg('char',strtolower($row['Type']))){
						if(	strtolower($this->fields[$i]->MySQLType)!=strtolower($row['Type'])
							&& strtolower($this->fields[$i]->MySQLType)!=ereg_replace('char','varchar',strtolower($row['Type']))
						) $tf=true;
		...
		function moverecord($direction=false){
			...
			$sql="SELECT ".$this->pos." FROM ".$this->name." WHERE ".$this->primary."=".vars('k');
			$sql.=" ".$this->filtersql;
			//echo $sql."<br>";
			...
			if(!empty($row2[$this->pos])){
				$sql="UPDATE ".$this->name." SET ".$this->pos."=".$row2[$this->pos]." WHERE ".$this->primary."=".vars('k');
				//echo $sql."<br>";
				mysql_query($sql);
