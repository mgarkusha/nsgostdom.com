<?php if (!defined('SCRIPTACCESS')) exit;
	class pexperience2 {
		...
		public function work(){
			$this->tpl=conf::$bpath.'admin/account_freelance/experience.tpl.php';
			global $auth;
			...
			$db=mysql::query("SELECT * FROM #freelance_experience WHERE account='?' ORDER BY `id`",$this->ide);
			$i=0;
		...
		public function store(){
			...
			$db=mysql::query("DELETE FROM #freelance_experience WHERE account='?'",$this->ide);
			foreach($place as $i=>$j){
				mysql::query("INSERT INTO #freelance_experience (`account`,`place`,`post`,`level`,`year1`,`year2`,`month1`,`month2`) VALUES ('?','?','?','?','?','?','?','?');",array($this->ide,$place[$i],$post[$i],$level[$i],$year1[$i],$year2[$i],$month1[$i],$month2[$i]));
		...
		public function display(){
			...
			<script>
				...
				function addelevel(num,value) {
					...
					input.setAttribute('name',name+'['+parseInt(num)+']');
					input.setAttribute('id',name+'['+parseInt(num)+']');
					input.className='inpututextarea';
					input.setAttribute('value',value);
					input.style.width = '100%';
					input.style.height = 60;
					s.appendChild(input);
					document.getElementById(name+'['+parseInt(num)+']').value=value;
				}
