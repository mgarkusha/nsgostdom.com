<?php if (!defined('SCRIPTACCESS')) exit;
	class peducation2 {
		...
		public function work(){
			//echo conf::$utype;
			$this->tpl=conf::$bpath.'admin/account_freelance/education.tpl.php';
			global $auth;
			if (conf::$utype==40 || conf::$utype==10)
				$this->ide=vars::mixed('e','int');
			else
				$this->ide=$auth->logged();
			$db=mysql::query("SELECT * FROM #freelance_education WHERE account='?' ORDER BY `id`",$this->ide);
			$i=0;
		...
		public function store(){
			...
			$db=mysql::query("DELETE FROM #freelance_education WHERE account='?'",$this->ide);
			foreach($place as $i=>$j){
				mysql::query("INSERT INTO #freelance_education (`account`,`place`,`speciality`,`level`,`type`,`year1`,`year2`) VALUES ('?','?','?','?','?','?','?');",array($this->ide,$place[$i],$speciality[$i],$level[$i],$type[$i],$year1[$i],$year2[$i]));
