<?php if (!defined('SCRIPTACCESS')) exit;
	class pexperience {
		...
		public function work(){
			...
			$db=mysql::query("SELECT * FROM #resume_experience WHERE account='?' ORDER BY `id`",$this->ide);
			$i=0;
		...
		public function store(){
			...
			$db=mysql::query("DELETE FROM #resume_experience WHERE account='?'",$this->ide);
			foreach($place as $i=>$j){
				mysql::query("INSERT INTO #resume_experience (`account`,`place`,`post`,`level`,`year1`,`year2`,`month1`,`month2`) VALUES ('?','?','?','?','?','?','?','?');",array($this->ide,$place[$i],$post[$i],$level[$i],$year1[$i],$year2[$i],$month1[$i],$month2[$i]));
